...
import { Send, User, Bot, BrainCircuit, AlertTriangle } from 'lucide-react'; // AlertTriangle for errors
    ...
    return (
      <div className="flex flex-col h-full items-center justify-center p-6 text-center bg-card">
        ...
        <p className="text-md text-muted-foreground">
          {currentLanguage === 'es' ? 'Usa el panel de la izquierda para empezar un nuevo chat o seleccionar uno existente.' : 'Use the panel on the left to start a new chat or select an existing one.'}
        </p>
      </div>
    );
  }
            ...
            >
              ...
              <div
                className={`max-w-[75%] md:max-w-[70%] rounded-xl px-4 py-3 shadow-md transition-all duration-200 ease-in-out ${
                  msg.type === 'user'
                    ...
                    : msg.isError
                      ? 'bg-destructive/20 text-destructive border border-destructive rounded-bl-none' // Adjusted for better visibility
              ...
              >
                {msg.isLoading ? (
                  ...
                  <>
                    {msg.isError && <AlertTriangle className="inline-block h-5 w-5 mr-2 text-destructive-foreground" />}
