...
export const studyAssistantFlow = ai.defineFlow(
  ...
  async (input: StudyAssistantInput): Promise<StudyAssistantOutput> => {
    ...
    const mainTextResponse = llmResponse.output?.directResponse || "No pude procesar tu solicitud en este momento.";
    const imageQueryFromLLM = llmResponse.output?.imageGenerationQuery;
    const followUps = llmResponse.output?.suggestedFollowUps;
    ...
    if (imageQueryFromLLM && imageQueryFromLLM.trim() !== "") {
      try {
        console.log(`Attempting to generate image with prompt: "${imageQueryFromLLM}"`);
